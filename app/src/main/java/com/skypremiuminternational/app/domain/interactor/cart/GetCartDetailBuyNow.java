...
public class GetCartDetailBuyNow extends UseCase<CartDetailResponse, Void> {

  @Inject
  protected GetCartDetailBuyNow(DataManager dataManager, ThreadExecutor subscriberThread,
  ...
  public Observable<CartDetailResponse> provideObservable(Void aVoid) {
    ...
    return getDataManager().createCart()
        .flatMap(ignored -> getDataManager().getCartDetailBuyNow()
            .map(this::flatInfo)
