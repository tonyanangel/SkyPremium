...
public class TravelDetailPresenter extends BasePresenter<TravelDetailView> {
  ...
  private GetOutletResevation getOutletResevation;

  TravelDetailViewState viewState;

  private CompositeSubscription compositeSubscription = new CompositeSubscription();

  @Inject
  public TravelDetailPresenter(GetAppVersion getAppVersion,
                               InternalStorageManager internalStorageManager, GetDetails getDetails,
                               ...
                               GetCategory getCategory, GetCartCount getCartCount , GetOutletResevation getOutletResevation) {
    super(getAppVersion, internalStorageManager);
    this.getCartCount = getCartCount;
    viewState = TravelDetailViewState.builder()
        .error(null)
        .isLoading(false)
        .isSuccess(false)
        .message(null)
  ...
  public void getDetailsSimple(final String sku,CategoryResponse categoryResponse) {
    onShowLoading();
    ...
    Subscription subscription = getDetails.execute(new BaseSubscriber<DetailsItem>() {
      ...
      public void onSuccess(DetailsItem response) {
        ...
        viewState = TravelDetailViewState.builder()
            ...
            .build();
        getView().render(viewState);
        addCategoryNames(response,categoryResponse);
        getView().renderDetailItem(response);
        DetailsItem detailsItem = viewState.message();
        String categoryRelation = TravelDetailActivity.category_id;
        ...
        getRecommendationList(sku,categoryRelation);
      }
  ...
  public void getRecommendationList(String sku,String category_id) {
    ...
    Subscription subscription = getFavourites.asObservable(GetFavouriteRequest.getAll())
        ...
        .subscribe(recommendItems -> getView().showRecommededList(recommendItems.getItems()),
            throwable -> {
              viewState = TravelDetailViewState.builder()
  ...
  public void setGetCategoryName(DetailsItem detailsItem) {
    addCategoryNames(detailsItem,viewState.category());
  }
  ...
  public void getCartCount() {
    add(getCartCount.asObservable()
        .subscribe(s -> getView().updateCartCount(s), throwable -> {
          viewState = TravelDetailViewState.builder()
              .error(new Exception("Failed to load cart item count"))
              .isLoading(false)
              .isSuccess(false)
              .category(viewState.category())
              .message(viewState.message())
