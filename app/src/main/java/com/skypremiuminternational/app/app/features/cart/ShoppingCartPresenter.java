...
public class ShoppingCartPresenter extends BasePresenter<ShoppingCartView> {
  ...
  @Inject
  ShoppingCartPresenter(GetAppVersion getAppVersion, InternalStorageManager internalStorageManager,
                        ...
                        GetCartDetailBuyNow getCartDetailBuyNow, GetCostBuyNow getCostBuyNow,
  ...
  void getLoyaltyPoints() {
    ...
    add(getUserDetailFromApi.asObservable()
        ...
        .subscribe(royaltyPoints -> {
          ShoppingCartPresenter.this.loyaltyPoints = royaltyPoints;
          Timber.e("loyalty");
          //getCartDetail();
        }, throwable -> {
          getView().hideLoading();
  ...
  void getCartDetail() {
    ...
    add(getCartDetail.execute(new SingleSubscriber<CartDetailResponse>() {
      ...
      public void onSuccess(CartDetailResponse response) {
        ShoppingCartPresenter.this.cartDetail = response;
  ...
  void getCartDetailBuyNow() {
    getView().showLoading("Get cart ...");
    add(getCartDetailBuyNow.execute(new SingleSubscriber<CartDetailResponse>() {
      @Override
      public void onSuccess(CartDetailResponse response) {
        updateBuyNowCart(response);
      }
  ...
  void updateBuyNowCart(CartDetailResponse response) {
    getView().showLoading("Get cart ...");
  ...
  void getCartDetailAfterChangeQtyBuyNow() {
    getView().showLoading("Get cart ...");
    add(getCartDetailBuyNow.execute(new SingleSubscriber<CartDetailResponse>() {
      @Override
      public void onSuccess(CartDetailResponse response) {
  ...
  public void deleteShoppingCartBuyNow(String cartItemId) {
    getView().showLoading("Deleting item ...");
    DeleteCartItem.Params params = new DeleteCartItem.Params(cartItemId);
    add(deleteCartItem.execute(new SingleSubscriber<Boolean>() {
      @Override
  ...
  private void getAllCost() {
    add(getAllCost.execute(new SingleSubscriber<CartAllInformationResponse>() {
      ...
      public void onSuccess(CartAllInformationResponse value) {
        ShoppingCartPresenter.this.allCost = value;
  ...
  private void getCostBuyNow() {
    add(getCostBuyNow.execute(new SingleSubscriber<CartAllInformationResponse>() {
      ...
      public void onError(Throwable error) {
        getView().hideLoading();
        getView().render(new Exception("Failed to load costs buy now"));
      }
     }));
  }
  ...
  void updateItemCountAfterChangeQtyBuyNow(CartDetailItem item, int qty) {
    getView().showLoading("Checking...");
    UpdateItemCountRequest request = buildRequest(item, qty);
    UpdateCartItemCount.Params params =
            new UpdateCartItemCount.Params(item, request);
    add(updateCartItemCount.execute(new SingleSubscriber<CartDetailItem>() {
      ...
      public void onSuccess(CartDetailItem value) {
        getView().hideLoading();
        //checkCartLimitForAdapter();
        getCartDetailAfterChangeQtyBuyNow();
      }
  ...
  void applyPromoCode(String promoCode,String from) {
    ...
    add(applyPromoCode.execute(new SingleSubscriber<Boolean>() {
      ...
      public void onSuccess(Boolean success) {
        getView().hideLoading();
        if (success) {
          if(checkCheckoutType(from)==CHECKOUT_TYPE_BUY_NOW){
            getCostBuyNow();
          }else {
            getAllCost();
          }
          getView().renderMessage("The promo code "+promoCode+" has been successfully applied to this order.");
        } else {
  ...
  void deleteRoyaltyPoints(String from) {
  ...
  void applyRoyaltyPoints(float royaltyPoints,String from) {
  ...
  void checkCartLimit(String from) {
    getView().showLoading("Checking limit...");
    add(checkLimit.execute(new SingleSubscriber<Boolean>() {
      @Override
      public void onSuccess(Boolean success) {
        getView().hideLoading();
        getView().proceedToCheckout(checkCheckoutType(from), checkPaymentType());
      }
  ...
  private int checkCheckoutType(String from) {
    ...
    } else if(from.equalsIgnoreCase(ShoppingCartActivity.BUY_NOW)){
      return CHECKOUT_TYPE_BUY_NOW;
